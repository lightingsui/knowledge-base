...
![](![](https://raw.githubusercontent.com/lightingsui/Pic/master/img/20200608233301.png)
...
`String s1 = new String("aaa");`和`String s5 = "aaa";`的区别在s1在堆中创建了对象，然后对中的值指向字符串常量池中的aaa，而s5则直接指向aaa，当采用s1方式创建时，如果字符串常量池中不存在aaa，则也会在字符串常量池中创建一个aaa。
...
![](https://raw.githubusercontent.com/lightingsui/Pic/master/img/20200608174925.png)
...
![](https://raw.githubusercontent.com/lightingsui/Pic/master/img/20200608175142.png)
...
复合赋值的规定：E1 op=E2等价于E1=(T)(E1 op E2)，自动进行隐式类型转换
...
Java 中有四个访问权限修饰符：private、protected 以及 public，如果不加访问修饰符，表示包级可见。

作用范围：

+ private：只对本类下可见
+ protected：只对本包下可见，如果子类不在本包下，也可见
+ default：只对本包下可见
+ public：全部可见
...
- 实现了接口就要实现接口中的全部方法（jdk1.8之前，jdk1.8之后接口可以有默认实现）
...
在很多情况下，接口优先于抽象类。**因为接口没有抽象类严格的类层次结构要求，可以灵活地为一个类添加行为。并且从 Java 8 开始，接口也可以有默认的方法实现，使得修改接口的成本也变的很低。（因为向接口中添加新方法需要在所有继承了该接口的类的中实现该方法，十分不方便）**
...
+ String.class
+ String a = new String("sdda");
  Class aClass = a.getClass();

**三种方式的优缺点**：

+ 第一种方法需要知道方法的全限定类名，适合获取jar包内对象的字节码，例如数据库的jdbc
+ 第二种方式直接根据类名进行获取，适用于自定义代码字节码的获取
+ 第三种就是通过类的实例对象去获取
...
Throwable（抽象类） 可以用来表示任何可以作为异常抛出的类，分为两种：  **Error**   和 **Exception**。其中 Error 用来表示 JVM 无法处理的错误，Exception 分为两种：
...
在finally中使用return，或吃掉try和catch中抛出的异常，并且会屏蔽掉try中的return；当在try中执行return时，会直接执行finally中return，提前结束掉，要完全避免在finally中使用return。

```java
public static int testFunc() {
        int a = 5;
        try {
            System.out.println("运行之前执行");
...
调用者就接收不到RuntimeException，被return吃掉了
